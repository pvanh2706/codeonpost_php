...
function upstock(itemid,tdid) {
    //$("td_"+tdid+"_btn").prop('disabled', true);
    var upstock_val = $("#td_data_"+tdid+"_update_stock").val();//'td_data_<?=$rowcount;?>_update_stock' 10 9
    var current_val = $("#td_data_"+tdid+"_current_stock").attr('data-current-stock');
    
    console.log('upstock_val: '+upstock_val + ' current_val: '+ current_val);
    var base_url = $("#base_url").val().trim();
    if (!upstock_val) {
        toastr["error"]("Chưa nhập số lượng điều chỉnh");
    } else {
        upstock_val = upstock_val - current_val;
        
    }
    console.log('upstock_val final: '+upstock_val);
                                                                    
                                                                    $.ajax({
                                                                            type: "GET",
                                                                            url: base_url+'items/updatestock/'+itemid+'/'+upstock_val,
...
$('#save,#update,#update-stock').on("click",function (e) {
  var this_id=this.id;
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    var tot_total_amt=$("#total_amt").text();
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				$.ajax({
				type: 'POST',
				url: base_url+'sales_stock/sales_save_and_update?command='+this_id+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
				data: data,
				cache: false,
				contentType: false,
				processData: false,
...
$("#item_search").autocomplete({
    ...
    source: function(data, cb){
        $.ajax({
          autoFocus:true,
            url: $("#base_url").val()+'items/get_json_items_details_stock',
            method: 'GET',
            dataType: 'json',
...
function return_row_with_data(item_id){
  $("#item_search").addClass('ui-autocomplete-loader-center');
	...
	$.post(base_url+"sales/return_row_with_data3/"+rowcount+"/"+item_id,{},function(result){
