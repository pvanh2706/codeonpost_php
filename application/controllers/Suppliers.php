...
class Suppliers extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $suppliers) {
			...
			$row[] = $suppliers->supplier_name . '<br><small style="display: none;">'.$suppliers->supplier_name_en.'</small>';
			$row[] = $suppliers->mobile;
			$row[] = $suppliers->email;
			$row[] = (!empty($suppliers->purchase_due) && $suppliers->purchase_due!=0) ? '<span class="label label-danger">' . app_number_format($suppliers->purchase_due) . '</span>' : (0) ;
						                     ...
						                     /* if($this->permissions('purchase_return_payment_add'))
						                      $str2.='<li>
						                        <a title="Pay Return Due" class="pointer" onclick="pay_return_due('.$suppliers->id.')" >
						                          <i class="fa fa-fw fa-money text-blue"></i>Nợ trả hàng
						                        </a>
						                      </li>';*/
											if($this->permissions('suppliers_edit'))
											$str2.='<li>
