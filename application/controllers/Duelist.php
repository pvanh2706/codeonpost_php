...
class Duelist extends MY_Controller {
	public function __construct(){
		parent::__construct();
		$this->load_global();
		$this->load->model('dues_model','sales');
	...
	public function index()
	{
		...
		$data=$this->data;
		$data['page_title']='Thống kê công nợ';
		$this->load->view('dues-list',$data);
	}
	public function add()
	{	
	...
	public function ajax_list()
	{
		...
		foreach ($list as $sales) {
			if ($sales->payment_status=='Unpaid' || $sales->payment_status=='Partial') {
			    $no++;
			$row = array();
			...
			$row[] = $sales->sales_code.$info;
			        ...
			        if ($sales->sales_status == 'Final') {
			            ...
			            $stt = "<span class='label label-warning' style='cursor:pointer'>Soạn hàng - Gửi Báo giá</span>";
			        } elseif($sales->sales_status == 'Shipping') {
			            //$stt = $sales->sales_status;
			            $stt = "<span class='label label-info' style='cursor:pointer'>Đang giao hàng</span>";
			        } else {
