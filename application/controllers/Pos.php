...
class Pos extends MY_Controller {
	...
	public function get_details_lv($lv){
		echo $this->pos_model->get_details_lv($lv);
	}
	...
	public function pos_save_temps(){
	    $result='';
	    if($this->input->post('command')=='update'){//Update
	    	$result = $this->pos_model->pos_save_update();
	    }
	...
	public function print_invoice_temp($sales_hold_id){
		$data=$this->data;
		$data['page_title']="In hóa đơn tạm";
		$data=array_merge($data,array('sales_id'=>$sales_hold_id));
		$this->load->view('sal-invoice-temp',$data);
	}
	public function public_invoice_aqua_home($sales_id){
		$data=$this->data;
		$data['page_title']='Aqua Home Invoice Public';
		//$data['page_title']=$this->lang->line('sales_invoice');
		$data=array_merge($data,array('sales_id'=>$sales_id));
		console_log($data);
		$this->load->view('sal-invoice-pos-public',$data);
	}
	public function get_item_details(){
		echo $this->pos_model->get_item_details($this->input->post('item_id'));
	}
	public function get_item_details_lv(){
		echo $this->pos_model->get_item_details_lv($this->input->post('item_id'), $this->input->post('item_lv'));
	}
