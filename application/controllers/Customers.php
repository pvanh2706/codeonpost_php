...
class Customers extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $customers) {
			...
			switch ($customers->customer_level) {
              case 0:
                $customerLevel = 'Đại lý Cấp 0';
                break;
              case 1:
                $customerLevel = 'Đại lý Cấp 1';
                break;
              case 2:
                $customerLevel = 'Đại lý Cấp 2';
                break;
            case 3:
                $customerLevel = 'Đại lý Cấp 3';
                break;    
              default:
                $customerLevel = 'Khách Lẻ';
            }
			
			$row[] = $customers->customer_code;
			$row[] = $customers->customer_name . '<br><small style="display: none;">'. $customers->customer_name_en .'</small>';
			$row[] = $customerLevel;
			$row[] = $customers->mobile;
			
			
			
			$row[] = app_number_format($customers->customer_point);
			...
			$row[] = ($customers->sales_return_due==null) ? (0) : app_number_format($customers->sales_return_due);
			 		...
			 		if($customers->status==1){ 
			 			$str= "<span onclick='update_status(".$customers->id.",0)' id='span_".$customers->id."'  class='label label-success' style='cursor:pointer'>Kích hoạt </span>";}
					else{ 
						$str = "<span onclick='update_status(".$customers->id.",1)' id='span_".$customers->id."'  class='label label-danger' style='cursor:pointer'>Vô hiệu hóa </span>";
					}
			$row[] = $str;			
