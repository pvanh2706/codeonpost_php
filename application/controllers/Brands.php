...
class Brands extends MY_Controller {
	...
	public function view(){
		...
		$data=$this->data;
		$data['page_title']='Danh sách thương hiệu';
	...
	public function ajax_list()
	{
		...
		foreach ($list as $brand) {
			...
			$row[] = ($brand->brand_code) ? $brand->brand_code : 'N/A';
			$row[] = '<span style="font-weight: bold;">'.$brand->brand_name.'</span>';
			$row[] = ($brand->description) ? $brand->description : 'N/A';

			 		if($brand->status==1){ 
			 			$str= "<span onclick='update_status(".$brand->id.",0)' id='span_".$brand->id."'  class='label label-success' style='cursor:pointer'>Kích hoạt </span>";}
					else{ 
						$str = "<span onclick='update_status(".$brand->id.",1)' id='span_".$brand->id."'  class='label label-danger' style='cursor:pointer'>Vô hiệu hóa </span>";
					}
			$row[] = $str;			
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a title="Edit Record ?" href="update/'.$brand->id.'">
													<i class="fa fa-fw fa-edit text-blue"></i>Sửa tên
												</a>
											</li>';
