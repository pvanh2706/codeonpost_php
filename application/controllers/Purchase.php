...
class Purchase extends MY_Controller {
		...
		foreach ($list as $purchase) {
			...
			$row[] = $purchase->purchase_code.$info;
			        ...
			        if ($purchase->purchase_status == 'Received') {
			            $stt = "<span class='label label-success' style='cursor:pointer'>Hoàn thành</span>";
			        } elseif($purchase->purchase_status == 'Pending') {
			            //$stt = $sales->sales_status;
			            $stt = "<span class='label label-warning' style='cursor:pointer'>Chờ xử lý</span>";
			        } elseif($purchase->purchase_status == 'Ordered') {
			            //$stt = $sales->sales_status;
			            $stt = "<span class='label label-info' style='cursor:pointer'>Đã đặt hàng</span>";
			        } else {
			            $stt = "<span class='label label-info' style='cursor:pointer'>Đang xử lý</span>";
			        }
			$row[] = $stt;
			$row[] = ($purchase->reference_no != null) ? $purchase->reference_no : 'N/A';
			$row[] = $purchase->supplier_name . '<br><small style="display: none;">' . $purchase->supplier_name_en .'</small>';
			...
			$row[] = ucfirst($purchase->created_by);
					$str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a title="View Invoice" href="purchase/invoice/'.$purchase->id.'" ><i class="fa fa-fw fa-eye text-blue"></i>Xem chi tiết
												</a>
											</li>';
											...
											$str2.='<li>
												<a title="Update Record ?" href="purchase/update/'.$purchase->id.'">
													<i class="fa fa-fw fa-edit text-blue"></i>Sửa đơn nhập
												</a>
											</li>';
											...
											$str2.='<li>
												<a title="Pay" class="pointer" onclick="pay_now('.$purchase->id.')" >
													<i class="fa fa-fw  fa-hourglass-half text-blue"></i>Thanh toán hết
												</a>
											</li>';
											...
											$str2.='<!--li>
												<a title="Purchase Return" href="purchase_return/add/'.$purchase->id.'">
													<i class="fa fa-fw fa-undo text-blue"></i>Hoàn trả hàng
												</a>
											</li-->';
											...
											$str2.='<li>
												<a style="cursor:pointer" title="Delete Record ?" onclick="delete_purchase(\''.$purchase->id.'\')">
													<i class="fa fa-fw fa-trash text-red"></i>Xóa đơn nhập
												</a>
											</li>
	...
	public function return_purchase_var_price($item_ids, $item_checkprice){
		echo $this->purchase->return_purchase_var_price($item_ids, $item_checkprice);
	}
	public function return_purchase_newprice_update($item_ids, $item_checkprice){
		echo $this->purchase->return_purchase_newprice_update($item_ids, $item_checkprice);
	}
