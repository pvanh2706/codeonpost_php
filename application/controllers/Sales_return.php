...
class Sales_return extends MY_Controller {
	...
	public function index()
	{
		...
		$data=$this->data;
		$data['page_title']="Thống kê Trả hàng";
  ...
  public function add($id){
		...
		if($q2->row()->sales_status=='Quotation'){
			$this->session->set_flashdata('Đơn hàng đang giao dịch không thể hoàn trả!');
		...
		if($q1->num_rows()>0){
			$this->session->set_flashdata('success','Đã thiết lập trạng thái đơn hoàn trả!');
			redirect(base_url('sales_return/edit/'.$q1->row()->id),'refresh');exit();
		}
	    
	    $data=$this->data;
	    $data['sales_id']=$id;;
	    $data['page_title']="Trả hàng hòan tiền";
