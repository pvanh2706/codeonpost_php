...
class Expense extends MY_Controller {
	...
	public function newexpense(){
		...
		$this->form_validation->set_rules('expense_for', 'Expense for', 'trim');
	...
	public function ajax_list()
	{
		...
		foreach ($list as $expense) {
			...
			//$row[] = ($expense->expense_for) ? $expense->expense_for : 'Không có';
			$row[] = app_number_format($expense->expense_amt);
			$row[] = ($expense->note) ? $expense->note : 'Không có ghi chú';			
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a style="cursor:pointer" title="Delete Record ?" onclick="delete_expense('.$expense->id.')">
													<i class="fa fa-fw fa-trash text-red"></i>Xóa bỏ
												</a>
											</li>
	...
	public function category_add()
	{
		...
		$data=$this->data;
		$data['page_title']='Thêm danh mục chi phí';
	...
	public function newcategory(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo "Vui lòng tạo tên danh mục.";
		}
	}
	public function ajax_list_expense()
	{
		...
		foreach ($list as $category) {
			...
			$row[] = ($category->description) ? $category->description : 'Không có ghi chú';

			 		if($category->status==1){ 
			 			$str= "<span onclick='update_status(".$category->id.",0)' id='span_".$category->id."'  class='label label-success' style='cursor:pointer'>Kích hoạt </span>";}
					else{ 
						$str = "<span onclick='update_status(".$category->id.",1)' id='span_".$category->id."'  class='label label-danger' style='cursor:pointer'>Vô hiệu hóa </span>";
					}
			$row[] = $str;			
					 $str2 = '<div class="btn-group" title="View Account">
										<a class="btn btn-primary btn-o dropdown-toggle" data-toggle="dropdown" href="#">
