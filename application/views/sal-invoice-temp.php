...
<!-- body onload="window.print();" -->
    ...
    $q3=$this->db->query("SELECT * FROM db_hold WHERE `id`='$sales_id'");
                          
    
    $res3=$q3->row();
    $customer_name=$res3->reference_id;
    $sales_date=show_date($res3->sales_date);
    $sales_code=$res3->sales_status;
...
<div class="invoices" id="invoices_image">
    ...
    <div class="row">
        <div class="col-12 text-center">
            <div style="font-size: 1.4em;  padding-top: 2px; font-weight: bold; text-align: center;">PHIẾU TẠM TÍNH</div>
            <div style="padding-bottom: 18px; font-weight: bold; text-align: center;"><?= $sales_date . ' ' . date("H:m:s",time()); ?></div>
        </div>
    </div>
		...
		<tr>
			<td>
				<table width="100%" style="font-size: 0.9em;">
				    <tr>
						<td>Mã phiếu tạm tính: TT<?= time() ?> - <?= $customer_name; ?></td>
					</tr>
				...
				<table width="100%" cellpadding="0" cellspacing="0"  >
					...
					<tbody style="border-bottom-style: dashed;border-width: 0.1px; font-size: 1em;">
						<?php
    			              ...
    			              $subtotal=0;
    			              $tax_amt=0;
    			              $q2=$this->db->query("select * from db_holditems where hold_id='$sales_id'");
    			              foreach ($q2->result() as $res2) {
    			                    
    			                    if ($res2->description) {
    			                        if ($res2->id == -1) {
    			                            echo "<tr ><td colspan='3' style='text-align:left; font-size: 0.9em'>".get_itemsname_from_id($res2->item_id)." - [".$res2->description."]</td></tr>"; 
    			                        } else {
    			                            echo "<tr ><td colspan='3' style='text-align:left; font-size: 0.9em'>".get_itemsname_from_id($res2->item_id)."</td></tr>"; 
					...
					<tr style="border-bottom-style: solid;border-top-style: solid;border-width: 0.1px;">
						<td style=" padding-left: 2px; padding-right: 2px;font-weight: bold;" colspan="2" align="left">Tổng tạm tính cần thanh toán</td>
