  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="<?php echo $base_url; ?>sales">Thống kê đơn hàng</a></li>
    ...
    $q3=$this->db->query("SELECT a.customer_name,a.mobile,a.phone,a.gstin,a.tax_number,a.email,
                           ...
                           b.sales_code,b.sales_status,b.sales_note,b.payment_status,
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            ...
            <small class="pull-right">Ngày lập: <?php echo  show_date($sales_date)." ".$created_time; ?></small>
          </h2>
        </div>
        <!-- /.col -->
      </div>
      ...
      <div class="row invoice-info">
        <div class="col-sm-4 invoice-col">
          ...
          <address>
            ...
            <?php //$this->lang->line('city'); ?><?php //echo  $company_city; ?>
        ...
        <div class="col-sm-4 invoice-col">
            <i>HỆ THỐNG<br></i>
          <b>Mã hóa đơn: <span style="color: blue;">#<?php echo  $sales_code; ?></span></b><br>
          <?php
                switch ($sales_status) {
                    case 'Shipping':
                        $stts = 'Đã xuất kho';
...
<hr>
        ...
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped records_table table-bordered">
            ...
            <tr>
              ...
              <th>Số lượng</th>
              <!--th>Giá trị</th-->
              <th class="<?=tax_disable_class()?>">Thuế</th>
              <th class="<?=tax_disable_class()?>">Giá trị thuế</th>
            ...
            <tbody>
              ...
              foreach ($q2->result() as $res2) {
                  ...
                  echo "<td>".number_format($res2->sales_qty)."</td>";

                  //echo "<td class='text-right'>".$CI->currency(number_format(($res2->price_per_unit * $res2->sales_qty)))."</td>";
                  ...
                  echo "<td class='text-right'>".number_format($res2->discount_amt, 0, ',', '.') . ' ₫'."</td>";
                  ...
                  echo "<td class='text-right'>".number_format($res2->total_cost, 0, ',', '.') . ' ₫'."</td>";
                  echo "</tr>";  
            ...
            <tfoot class="text-right text-bold bg-gray">
              <tr>
                ...
                <td><?= number_format($tot_sales_price, 0, ',', '.') . ' ₫';?></td>
                <td class="text-left"><?=number_format($tot_qty);?></td>
                <td class="<?=tax_disable_class()?>"></td>
                <td class="<?=tax_disable_class()?>"></td>
                <!--td><?= $CI->currency(number_format($tot_tax_amt));?></td-->
                <!--td></td-->
                <td><?= number_format($tot_discount_amt, 0, ',', '.') . ' ₫' ;?></td>
                <!--td></td-->
                <td><?= number_format($tot_total_cost, 0, ',', '.') . ' ₫' ;?></td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      <div class="row">
       <div class="col-md-6">
          ...
          <div class="row">
              <div class="col-md-12">
                 <div class="form-group">
                    <label for="sales_note" class="col-sm-4 control-label" style="font-size: 17px;">Ghi chú hóa đơn</label>    
                    <div class="col-sm-8">
                       <label class="control-label  " style="font-size: 17px;">: <?=($sales_note) ? $sales_note : 'Không có ghi chú kèm theo';?></label>
                    </div>
                 </div>
              </div>
           </div> 
                 ...
                 <div class="form-group">
                    ...
                    <table  class="col-md-11">
                       ...
                       <tr>
                          ...
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h4><b id="discount_to_all_amt" name="discount_to_all_amt"><?=number_format($tot_discount_to_all_amt, 0, ',', '.') . ' ₫';?><?php 
                                if($discount_to_all_input > 0) {
                                   if($discount_to_all_type == '%') {
                                      echo ' <small class="text-muted">('.$discount_to_all_input.'%)</small>';
                                   } else {
                                      echo ' <small class="text-muted">(Cố định)</small>';
                                   }
                                }
                             ?></b></h4>
                          </th>
                       ...
                       <tr style="border-top: 1px solid #ddd;">
                          <th class="text-right" style="font-size: 17px;">Tổng sau chiết khấu</th>
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
                             <h4><b><?php 
                       ...
                       <tr style="border-top: 2px solid #333;">
                          <th class="text-right" style="font-size: 17px; font-weight: bold;">TỔNG THANH TOÁN</th>
                          <th class="text-right" style="padding-left:10%;font-size: 17px;">
      ...
      <div class="row no-print text-right">
        <div class="col-xs-12">
            <?php if($CI->permissions('sales_edit')) { ?>
                <?php $str2= ($pos==1)? 'pos/edit/':'sales/update/'; ?>
                    <!--a href="<?php echo $base_url; ?><?=$str2;?><?php echo  $sales_id ?>" class="btn btn-success"><i class="fa  fa-edit"></i> Edit </a-->
                <?php } ?>
        ...
        <!--a href="<?php echo $base_url; ?>sales/pdf/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary"><i class="fa fa-file-pdf-o"></i>  PDF </a-->
...
<script>
  ...
  //var base_url=$("#base_url").val();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $.post('<?= base_url();?>sales/save_payment', {sales_id: sales_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
