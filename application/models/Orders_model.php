...
class Orders_model extends CI_Model {
	...
	var $column_order = array( 'a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','b.customer_name_en','a.paid_amount','a.sales_status','a.pos'); //set column field database for datatable orderable
	var $column_search = array('sales_due','a.return_bit','a.id','a.sales_date','a.sales_code','a.reference_no','a.grand_total','a.payment_status','a.created_by','b.customer_name','b.customer_name_en','a.paid_amount','a.sales_status','a.pos'); //set column field database for datatable searchable 
	...
	private function _get_datatables_query()
	{
		...
		if(!empty($customer_id)){
	     ...
	     $sales_stt = $this->input->post('user_created_by');
	     if(!empty($sales_stt)){
	         $stts = array("Quotation", "Final", "Shipping");
	         if (in_array($sales_stt, $stts)) {
	             $this->db->where('a.sales_status',$sales_stt);
	         } else {
	             if ($sales_stt == 'Paid'){
	                 $this->db->where('a.payment_status',$sales_stt);
	             } else {
	                 $names = array('Unpaid', 'Partial');
                     $this->db->where_in('a.payment_status', $names);
	                 //$this->db->where('a.payment_status',$sales_stt);
	             }
	...
	public function verify_save_and_update(){
	    ...
	    if($command=='save'){//Create sales code unique if first time entry
			...
			$maxid=$q4->row()->maxid;
			$sales_code=$sales_init.time().str_pad($maxid, 6, '0', STR_PAD_LEFT);
			
			//echo "<pre>";print_r($sales_note);exit();
            ...
            //log_customer_point($customer_id,$sales_code,$getPoint,'Tăng mới điểm tích lũy');
		}
			...
			if($q6->num_rows()>0){
				...
				//add_customer_point($customer_id,$sales_point);
                //log_customer_point($customer_id,$sales_id,$getPoint,'Cập nhật thêm điểm tích lũy');
			}
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$description		=$this->xss_html_filter(trim($_REQUEST['itemdescription_'.$i]));
		...
		if($amount>0 && !empty($payment_type)){
			...
			if($q3!=1){
			    ...
			    $customer_id = get_customer_id_by_sales($sales_id);
    			$customer_point = get_customer_point($customer_id);
                ...
                $sales_point = array('customer_point' => $point_add);
                            ...
                            if ($point_system == 1) {
                                add_customer_point($customer_id,$sales_point);
                                log_customer_point($customer_id,'Sales_model: '.$command.' | Sales Code: '.$sales_code,$customer_point,$getPoint,'Tăng mới '. $getPoint .' điểm tích lũy từ hóa đơn ' . $sales_code );
                            }
	...
	public function paid_sales($ids){
	    $this->db->trans_begin();
	    
	    $q12=$this->db->select("*")->where("id in ($ids)")->get("db_sales");
      	if($q12->num_rows()>0){
      		foreach ($q12->result() as $res12) {
      		    $sales_status = $res12->sales_status;
      		    if ($sales_status == 'Final') {
      		        $sales_id = $res12->id;
          		    $customer_id = $res12->customer_id;
          		    $grand_total = $res12->grand_total;
    	            $paid_amount = $res12->paid_amount;
    	            
    	            $amount = $grand_total - $paid_amount;
          		    $payment_type = 'Tiền mặt';
          		    $payment_date = date('dd-mm-Y');
          		    $payment_note = 'Chức năng thanh toán nhanh nhiều đơn hàng';
          			//do something
                	...
                	if($amount=='' || $amount==0){$amount=null;}
                        ...
                        $sales_point = array('customer_point' => $point_add);
                                        ...
                                        if($point_system == 1) {
            						        add_customer_point($customer_id,$sales_point);
                                            log_customer_point($customer_id,'Sales_model: save_payment() | Sales Code: '.$sales_code,$customer_point,$getPoint,'Tăng mới '.number_format($getPoint).' điểm tích lũy cho hóa đơn ' . $sales_code);
                                        }
	...
	public function get_items_info2($cusLvs,$rowcount,$item_id){
		$q1=$this->db->select('*')->from('db_items')->where("id=$item_id")->get();
		$q3=$this->db->query("select * from db_tax where id=".$q1->row()->tax_id)->row();
		...
		switch ($cusLvs) {
		    case 0:
		        $info['item_sales_price'] = $q1->row()->final_price0;
		        break;
		    case 1:
		        $info['item_sales_price'] = $q1->row()->final_price1;
		        break;
		    case 2:
		        $info['item_sales_price'] = $q1->row()->final_price2;
		        break;
		    case 3:
		        $info['item_sales_price'] = $q1->row()->final_price3;
		        break;
		    default:
		        $info['item_sales_price'] = $q1->row()->sales_price;
		}
		...
		$info['item_p0'] = $q1->row()->final_price0;
		$info['item_p1'] = $q1->row()->final_price1;
		$info['item_p2'] = $q1->row()->final_price2;
		$info['item_p3'] = $q1->row()->final_price3;
		$info['item_p'] = $q1->row()->final_price;
	...
	public function get_items_info3($rowcount,$item_id){
		$q1=$this->db->select('*')->from('db_items')->where("id=$item_id")->get();
		//$q3=$this->db->query("select * from db_tax where id=".$q1->row()->tax_id)->row();
		...
		$this->return_row_with_data3($rowcount,$info);
	}
	...
	public function return_row_with_data($rowcount,$info){ // đang sử dụng
		extract($info);
		$item_amount = ($item_sales_price * $item_sales_qty) + $item_tax_amt;
		?>
            <tr style="table-layout: fixed;" class="itemsrows" id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>' data-item-id="<?=$item_id;?>" data-p0="<?=number_format($item_p0);?>" data-p1="<?=number_format($item_p1);?>" data-p2="<?=number_format($item_p2);?>" data-p3="<?=number_format($item_p3);?>" data-p="<?=number_format($item_p);?>">
               <td id="td_<?=$rowcount;?>_1">
                  <label class='form-control' style='height:auto;' data-bs-toggle="tooltip" title='Xem các mức giá niêm yết' >
                  <!--a id="td_data_<?=$rowcount;?>_1" href="javascript:void(0)" onclick="show_sales_item_modal(<?=$rowcount;?>)" title=""><?=$item_name;?></a--> 
                  <a id="td_data_<?=$rowcount;?>_1" href="javascript:void(0)" onclick="show_sales_item_modal_price(<?=$rowcount;?>)" title=""><?=$item_name;?> </a> 
               ...
               <td id="td_<?=$rowcount;?>_3">
                  <div class="input-group ">
                     ...
                     <input onclick="this.select();" type="number" value="<?=$item_sales_qty;?>" class="form-control no-padding text-center" onchange="item_qty_input(<?=$rowcount;?>)" id="td_data_<?=$rowcount;?>_3" name="td_data_<?=$rowcount;?>_3">
                     <span class="input-group-btn">
            ...
            <tr id="doublerow_<?=$rowcount;?>" style="table-layout: fixed;">
                    <td colspan="6">
                        <?php
                            $CTB = '';
                            if ($item_id != -1) {
                                $CTB = 'Ghi chú thêm cho sản phẩm '. $item_name .' (SL có thể bán: '.number_format(cansold($item_id)).')';
                            } ?>
                        <input name="itemdescription_<?=$rowcount;?>" class='form-control' type="text" value="<?=$description;?>" placeholder="<?= $CTB ?>">
                    </td>
                </tr>
		<?php
	...
	public function return_row_with_data3($rowcount,$info){
		extract($info);
		//$item_amount = ($item_sales_price * $item_sales_qty) + $item_tax_amt;
		?>
            <tr class="itemsrows" id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>' data-p0="<?=number_format($item_p0);?>" data-p1="<?=number_format($item_p1);?>" data-p2="<?=number_format($item_p2);?>" data-p3="<?=number_format($item_p3);?>" data-p="<?=number_format($item_p);?>">
                <!--td id="td_<?=$rowcount;?>_1001">
                   <label class="form-control">
                       <?=$item_id;?>
                   </label>
                </td-->
                <td id="td_<?=$rowcount;?>_1">
                  <label class='form-control' style='height:auto;' data-toggle="tooltip" title='Xem các mức giá niêm yết' >
                     <a id="td_data_<?=$rowcount;?>_1" href="javascript:void(0)" onclick="show_sales_item_modal_price(<?=$rowcount;?>)" title=""><?=$item_name;?></a>
                  </label>
                </td>
               
                <td id="td_<?=$rowcount;?>_10">
                   <input readonly type="text" data-current-stock="<?=$item_stock?>" name="td_data_<?=$rowcount;?>_current_stock" id="td_data_<?=$rowcount;?>_current_stock" class="form-control text-right no-padding only_currency text-center" value="<?=number_format($item_stock);?>">
                </td>
                ...
                <td>
                    <button onclick="upstock(<?=$item_id;?>,<?=$rowcount;?>)" type="button" id="td_<?=$rowcount;?>_btn" name="td_<?=$rowcount;?>_btn" class="btn bg-green btn-block" >Xác nhận</button>
                    
                </td>
                <td>
                    <button onclick="removerow(<?=$rowcount;?>)" type="button" id="td_data_<?=$rowcount;?>_16" name="td_data_<?=$rowcount;?>_16" class="btn bg-red btn-block" >Xóa</button>
                </td>
                <input type="hidden" id="td_<?=$rowcount;?>_1001" name="td_<?=$rowcount;?>_1001" value="<?=$item_id;?>" >
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        ...
		        <h4 class="modal-title text-center">Thanh toán đơn #<?php echo  $sales_code; ?></h4>
		      </div>
		      	...
		      	<div class="row invoice-info">
			        <div class="col-sm-6 invoice-col">
			          ...
			          <address>
			            ...
			            <?php echo (!empty(trim($customer_mobile))) ? "Liên hệ: ".$customer_mobile."<br>" : '';?><br>
			            Hóa đơn #<?php echo  $sales_code; ?>
			          </address>
			        </div>
			        <!-- /.col -->
			        <div class="col-sm-6 invoice-col">
			          <address>
			            <b>Ngày: <?= show_date($sales_date); ?></b><br>
			            <b>Tổng thanh toán: <span style="color: blue;"><?php echo number_format($grand_total); ?></span></b><br>
			            <hr>
			            <b>Đã thanh toán: <span><?php echo number_format($paid_amount); ?></span></b><br>
			            <b>Công nợ còn: <span id='due_amount_temp' style="color: red;" data-due_amount_temp="<?php echo $due_amount; ?>"><?php echo number_format($due_amount); ?></span></b><br>
			          </address>
			        </div>
			        <!-- /.col -->
			      </div>
		          ...
		          <div class="box box-solid bg-gray">
		            <div class="box-body">
		              <div class="row">
		                ...
		                <div class="col-md-4">
		                  <div class="">
		                  <label for="amount">Thanh toán</label>
		                ...
		                <div class="col-md-4">
		                  <div class="">
		                    <label for="payment_type">Hình thức</label>
	...
	public function show_stt_now_modal($sales_id){
		$q1=$this->db->query("select * from db_sales where id=$sales_id");
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2=$this->db->query("select * from db_customers where id=$customer_id");
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Hoàn thành đơn #<?php echo  $sales_code; ?></h4>
		      </div>
		      <div class="modal-body">
		      	...
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-6 invoice-col">
			          <address>
			            ...
			            <hr>
			            <b>Đã thanh toán: <span><?php echo number_format($paid_amount); ?></span></b><br>
			            <b>Công nợ còn: <span id='due_amount_temp' style="color: red;"><?php echo number_format($due_amount); ?></span></b><br>
			          </address>
			        </div>
			        <!-- /.col -->
			      </div>
	...
	public function show_stt_final_now_modal($sales_id){
		$q1=$this->db->query("select * from db_sales where id=$sales_id");
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2=$this->db->query("select * from db_customers where id=$customer_id");
	    ...
	    $reference_no=$res1->reference_no;
	    $sales_code=$res1->sales_code;
	    $sales_status=$res1->sales_status;
	    $sales_note=$res1->sales_note;
	    $grand_total=$res1->grand_total;
	    $paid_amount=$res1->paid_amount;
	    $due_amount =$grand_total - $paid_amount;
	    ...
	    switch ($sales_status) {
	        case 'Quotation':
	            $sales_status = "Đang giao dịch";break;
	        case 'Final':
	            $sales_status = "Đã giao hàng";break;
	        case 'Shipping':
	            $sales_status = "Đã xuất kho";break;
	        default:
	            $sales_status = "Đang xử lý";
	    }
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Chuyển trạng thái đơn #<?php echo $sales_code; ?></h4>
		      </div>
		      <div class="modal-body">
		    ...
		    <div class="row">
		        <div class="col-md-12">
		            Chuyển trạng thái đơn từ <span style="font-weight:bold; font-size:1.2em;"><?php echo $sales_status; ?></span> thành <span style="font-weight:bold; font-size:1.2em;"> Đã giao hàng </span>
		        </div>
	...
	public function show_stt_shipping_now_modal($sales_id){
		$q1=$this->db->query("select * from db_sales where id=$sales_id");
		$res1=$q1->row();
		$customer_id = $res1->customer_id;
		$q2=$this->db->query("select * from db_customers where id=$customer_id");
		...
		<div class="modal fade" id="pay_now">
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		        <div class="col-md-12">
		            Chuyển trạng thái đơn từ <span style="font-weight:bold; font-size:1.2em;"><?php echo $sales_status; ?></span> thành <span style="font-weight:bold; font-size:1.2em;"> Đã xuất kho </span>
		        </div>
	...
	public function save_stt(){
		extract($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		$q10 = $this->db->query("update db_sales set sales_status='Final' where id='$sales_id'");
		$q100 = $this->db->query("update db_salesitems set sales_status='Final' where sales_id='$sales_id'");
		if($q10!=1){
			return "failed";
		}
		return "success";
	...
	public function save_stt_shipping(){
		extract($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		$q10 = $this->db->query("update db_sales set sales_status='Shipping' where id='$sales_id'");
		if($q10!=1){
			return "failed";
		}
		return "success";
		  ...
		  <div class="modal-dialog modal-lg">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Lịch sử thanh toán</h4>
		      </div>
		      <div class="modal-body">
                                ...
                                <tbody>
									...
									if($q1->num_rows()>0){
										foreach ($q1->result() as $res1) {
											...
											echo "<td>".number_format($res1->payment)."</td>";
											echo "<td>".$res1->payment_type."</td>";
											//echo "<td>".$res1->payment_note."</td>";
											//echo "<td>".ucfirst($res1->created_by)."</td>";
										
											echo "<td><a onclick='delete_sales_payment(".$res1->id.")' class='pointer btn  btn-danger' >Xóa bỏ</td>";	
											echo "</tr>";
											echo "<tr colspan='6'>Ghi chú: ".$res1->payment_note."</tr>";
										}
									}
									else{
