...
class Login_model extends CI_Model
{
	...
	public function verify_credentials($username,$password)
	{
		...
		$query=$this->db->query("select a.validate, a.id,a.username,a.role_id,b.role_name from db_users a, db_roles b where b.id=a.role_id and  a.username='$username' and a.password='".md5($password)."' and a.status=1");
		if($query->num_rows()==1){
            /*if ($query->row()->validate < time()) {
                $this->session->set_flashdata('failed', 'Tài khoản sử dụng đến ' . date("d-m-Y H:i:s",$query->row()->validate));
                return false;
            }*/
			$logdata = array('inv_username'  => $query->row()->username,
				        	 'inv_userid'  => $query->row()->id,
				        	 'validate'     => $query->row()->validate,
				        	 'logged_in' => TRUE,
			...
			$this->session->set_flashdata('success', 'Xin chào '.ucfirst($query->row()->username)." <br> Hạn sử dụng đến " . date("d-m-Y H:i:s",$query->row()->validate));
			return true;
		}
		else{
		    $this->session->set_flashdata('failed', 'Sai thông tin đăng nhập. - ' . time());
			return false;
		}		
	}
