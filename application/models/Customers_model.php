...
class Customers_model extends CI_Model {
	...
	var $column_order = array('a.customer_code','a.id','a.customer_name','a.customer_name_en','customer_point','customer_level','a.mobile','a.email','a.status','a.sales_due','a.sales_return_due'); //set column field database for datatable orderable
	var $column_search = array('a.customer_code','a.id','a.customer_name','a.customer_name_en','customer_point','customer_level','a.mobile','a.email','a.status','a.sales_due','a.sales_return_due'); //set column field database for datatable searchable 
	...
	public function verify_and_save(){
		...
		if($query2->num_rows()>0 && !empty($mobile)){
			return "Số điện thoại liên hệ này đã tồn tại.";;
		}
		...
		$query1="insert into db_customers(customer_code,customer_name,customer_point,customer_level,mobile,phone,email,
											...
											values('$customer_code','$customer_name','$customer_point','$customer_level','$mobile','$phone','$email',
											'$country',$state,'$city','$postcode','$address','$opening_balance',
											'$SYSTEM_IP','$SYSTEM_NAME',
											'$CUR_DATE','$CUR_TIME','$CUR_USERNAME',1,'$gstin','$tax_number')";

		if ($this->db->simple_query($query1)){
				$this->session->set_flashdata('success', 'Đã thêm khách hàng mới!');
		        return "success";
		}
		else{
		        return "failed";
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['customer_point']=$query->customer_point;
			$data['customer_level']=$query->customer_level;
	...
	public function update_customers(){
		...
		if($q_id==1){
			echo "Không thể cập nhật! Hỏi thằng Long xem bị gì nha!";exit();
		}
		...
		if($query->num_rows()>0){
			...
			$query1="update db_customers set customer_name='$customer_name',customer_point='$customer_point',customer_level='$customer_level',mobile='$mobile',phone='$phone',
							...
							postcode='$postcode',address='$address',gstin='$gstin',tax_number='$tax_number'
							 where id=$q_id";
			$current_customer_point = get_customer_point($q_id);
			if ($this->db->simple_query($query1)){
			        if ($customer_point != $current_customer_point) {
			            log_customer_point($q_id, 0, $current_customer_point, $customer_point, '* Admin điều chỉnh tích điểm của thành viên từ ' . number_format($current_customer_point) . ' >> ' . number_format($customer_point));
			        }
					$this->session->set_flashdata('success', 'Đã cập nhật thông tin khách hàng!');
			        return "success";
			}
			else{
			        return "failed";
	...
	public function delete_customers_from_table($ids){
		...
		if($q1->row()->tot_entrys >0 ){
			echo "Khách hàng này đã từng mua hàng, không xóa được nha!";exit();
		}
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        ...
		        <h4 class="modal-title text-center">Thanh toán công nợ đến hạn</h4>
		      </div>
		    ...
		    <div class="row">
		      <div class="col-md-12">
		      	<div class="row invoice-info">
			        <div class="col-sm-12 invoice-col">
			          ...
			          <address>
			            Khách hàng: <strong> <?php echo  $customer_name; ?></strong><br>
			            <?php echo (!empty(trim($customer_mobile))) ? "Liên hệ: <strong>".$customer_mobile."</strong><br>" : '';?>
			            <?php echo (!empty(trim($customer_email))) ? "Email: <strong>".$customer_email."</strong><br>" : '';?>
			        ...
			        <div class="col-sm-12 invoice-col">
			        	...
			        	<table class="table table-sm table-bordered bg-info" width="100%">
			        		<tr>
			        			...
			        			<td class="text-right">Công nợ tổng</td>
			        			<td class="text-right"><?= $CI->currency(number_format($total_sales_amount)); ?></td>
			        		</tr>
			        		<tr>
			        			...
			        			<td class="text-right"><?= $CI->currency(number_format($total_paid_amount)); ?></td>
			        		</tr>
			        		<tr>
			        			<td class="text-right">Công nợ còn lại</td>
			        			<td class="text-right" style="font-weight: bold; color: red;"><?= $CI->currency(number_format($customer_sales_due)); ?></td>
			        		</tr>
			        	</table>
		          ...
		          <div class="box box-solid bg-gray">
		            <div class="box-body">
		              <div class="row">
		         		<div class="col-md-4">
		                  <div class="">
		                  <label for="payment_date">Thời gian</label>
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$customer_id;?>)" class="btn bg-green btn-lg place_order btn-lg payment_save">Lưu<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
	    	...
	    	while($amount>0) {
	    		...
	    		if($amount<=$customer_sales_due){
	    			...
	    			foreach ($qs4->result() as $res) {
                        ...
                        $getPoint = floor(($getAmount / $minpaid_get_point) * $point_get_per_minpaid);
                        $point_add = $customer_point + $getPoint;
                        $sales_point = array('customer_point' => $point_add);
                        
                        if ($point_system == 1){
                            add_customer_point($customer_id,$sales_point);
                            log_customer_point($customer_id,'Customers_model: save_payment | Sales Code: '.$sales_code,$customer_point,$getPoint,'Tăng mới '.number_format($getPoint).' điểm tích lũy từ trả nợ hóa đơn ' . $sales_code);
                        }
		  ...
		  <div class="modal-dialog ">
		    <div class="modal-content">
		      <div class="modal-header header-custom">
		        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		          <span aria-hidden="true">&times;</span></button>
		        <h4 class="modal-title text-center">Pay Sales Return Due Payments Tèn ten</h4>
		      </div>
		      <div class="modal-body">
		    ...
		    <div class="row">
		      ...
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default btn-lg" data-dismiss="modal">Đóng</button>
		        <button type="button" onclick="save_return_due_payment(<?=$customer_id;?>)" class="btn bg-green btn-lg place_order btn-lg return_due_payment_save">Lưu<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
	...
	public function getCustomersArray($id=''){
		...
		$this->db->select("id, customer_name, mobile, customer_level, customer_point, address")->from('db_customers');
		...
		if(!empty($id)){
			...
			$q = (isset($_POST['searchTerm'])) ? $_POST['searchTerm'] : '';
			//$q = str_replace(' ', '%'. $_POST['searchTerm']);
			//$q = '%'.$q.'%';
            //$q = permalink($q);
			$this->db->where("(upper(customer_name) like '%$q%' or upper(customer_name_en) like '%$q%' or upper(mobile) like '%$q%')");
			//$this->db->where("customer_name_en like '%$q%' or customer_name like '%$q%'");
			//vi_to_en
		}
		//$this->db->limit(10);
		...
		if($query->num_rows()>0){
			foreach($query->result() as $res){
				...
				$json_arr["id"] 					 = $res->id;
			  	...
			  	$json_arr["level"] 				 = $res->customer_level;
			  	$json_arr["point"] 				 = number_format($res->customer_point);
			  	$json_arr["address"]            =  $res->address; 
