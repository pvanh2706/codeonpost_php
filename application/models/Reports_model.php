	...
	public function show_stock_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$color = ($res1->stock == 0) ? 'color: red; ' : '';
				$str.="<tr>";
				$str.="<td>".++$i."</td>";
				//$str.="<td>".$res1->item_code."</td>";
				$str.="<td>".$res1->item_name." <span style='display: none;'>".$res1->search_for ."</span></td>";
				$str.="<td style='" . $color . "font-weight: bold;'>".$res1->stock."</td>";
				
				$str.="<td style='" . $color . "font-weight: bold;'>".cansold($res1->id)."</td>";
				$str.="<td style='" . $color . "font-weight: bold;'>".app_number_format($res1->stock * $res1->purchase_price)."</td>";
				...
				$str.="<td class='text-right'>".app_number_format($res1->final_price0)."</td>";
				$str.="<td class='text-right'>".app_number_format($res1->final_price1)."</td>";
				$str.="<td class='text-right'>".app_number_format($res1->final_price2)."</td>";
				$str.="<td class='text-right'>".app_number_format($res1->final_price3)."</td>";
				//$str.="<td class='text-right'>".$stock_value."</td>";
				$str.="<td class='text-right'><a class='btn btn-success' href='" . base_url('items/update/'.$res1->id). "' target='_Blank'>Điều chỉnh</a></td>";
				$str.="</tr>";
...
//		$this->db->group_by("c.`item_id`");
	...
	public function get_profit_loss_report(){
            ...
            //"select sum(stock * purchase_price) as totalvon from db_items where stock > 0 and id > 0"
            //$this->db->select("sum(stock * purchase_price) as opening_stock_price");
			//$this->db->from("db_items");
			//$this->db->where("stock" > 0);
			//$this->db->where("id" > 0);
			
			
            $opening_stock_price = $this->db->query("select sum(stock * purchase_price) as opening_stock_price from db_items where stock > 0 and id > 0")->row()->opening_stock_price;
            $info['opening_stock_price']=number_format($opening_stock_price,0);
	...
	public function get_profit_by_invoice(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td><a target='_Blank' href='".base_url("sales/invoice/$res1->id")."'>".$res1->sales_code."</a></td>";
				echo "<td>".show_date($res1->sales_date)."</td>";
				echo "<td>".$res1->customer_name."</td>";
