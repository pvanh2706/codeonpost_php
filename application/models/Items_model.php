...
class Items_model extends CI_Model {
	...
	var $column_order = array( 'a.id','a.item_image','a.item_code','a.item_name','a.search_for','b.category_name','c.unit_name','a.stock','a.alert_qty','a.purchase_price','a.final_price','a.final_price0','a.final_price1','a.final_price2','a.final_price3','d.tax_name','d.tax','a.status','e.brand_name','a.tax_type','a.hsn','a.sku'); //set column field database for datatable orderable
	var $column_search = array( 'a.id','a.item_image','a.item_code','a.item_name','a.search_for','b.category_name','c.unit_name','a.stock','a.alert_qty','a.purchase_price','a.final_price','a.final_price0','a.final_price1','a.final_price2','a.final_price3','d.tax_name','d.tax','a.status','e.brand_name','a.custom_barcode','a.tax_type','a.hsn','a.sku'); //set column field database for datatable searchable 
	...
	/*public function up_stock($id,$sl){
	    if ($this->db->simple_query("update db_items set stock = stock +".$sl." where id =".$id)) {
	...
	private function _get_datatables_query()
	{
		...
		$items_name_s = $this->input->post('items_name_s');
		if(!empty($brand_id)){
        ...
        if(!empty($items_name_s)){
			$this->db->or_like('a.item_name', str_replace(" ", "%", $items_name_s), 'both', false);
			$this->db->or_like('a.search_for', str_replace(" ", "%", $items_name_s), 'both', false);
			//$this->db->or_like("a.item_name", str_replace(" ", "%" , $items_name_s), "both");
		}
    ...
    public function getItemsPriceJson($itemid,$plevel){
		return json_encode($this->getItemsPriceArray($itemid,$plevel));
	}
	public function getItemsPriceArray($itemid='',$plevel=''){
	    $colname = '';
	    switch ($plevel) {
	        case 0:
	            $colname = 'final_price0';
	            break;
	           case 1:
	            $colname = 'final_price1';
	            break;
	            case 2:
	            $colname = 'final_price2';
	            break;
	            case 3:
	            $colname = 'final_price3';
	            break;
	            default:
	                $colname = 'final_price';
		...
		$this->db->select($colname)->where('id',$itemid)->from('db_items');
		...
		if($query->num_rows()>0){
			foreach($query->result() as $res){
                //$json_arr['itemid'] 					 = $res->$colname;
				$json_arr['itemprice'] 					 = $res->$colname;
	...
	public function verify_and_save(){
		...
		$query1="insert into db_items(description,item_code,item_name,brand_id,category_id,sku,hsn,unit_id,alert_qty,lot_number,expire_date,
									...
									sales_price,custom_barcode,final_price,final_price0,final_price1,final_price2,final_price3,
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['final_price0']=$query->final_price0;
			$data['final_price1']=$query->final_price1;
			$data['final_price2']=$query->final_price2;
			$data['final_price3']=$query->final_price3;
	...
	public function getItemsArray($id=''){
		...
		$this->db->select("id, item_name, item_code, stock")->from('db_items')->where('id >', '0');
		...
		if(!empty($id)){
			...
			$this->db->where("(upper(item_name) like '%$q%' or upper(item_code) like '%$q%' or upper(search_for) like '%$q%')");
		}
		//$this->db->limit(100);
