...
class Dashboard_model extends CI_Model
{
	...
	public function breadboard_values()
	{
	    $ldate = date("Y-m-d");
	    $thismonth = date("m");
	    $query10000="select COALESCE(sum(grand_total),0) as total_day from db_sales where sales_date = '".$ldate."'";
		$total_day=$this->db->query($query10000)->row()->total_day;	
		
		$query67="SELECT COALESCE(sum(grand_total),0) AS tot_sal_grand_total_this_month FROM db_sales where MONTH(sales_date) = '$thismonth'";
		$tot_sal_grand_total_this_month=$this->db->query($query67)->row()->tot_sal_grand_total_this_month;
        ...
        $data['total_month'] = $tot_sal_grand_total_this_month;
