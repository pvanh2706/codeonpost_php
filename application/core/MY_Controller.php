...
class MY_Controller extends CI_Controller{
     // public $source_version = app_version();
      ...
      public function load_info(){
            ...
            $query =$this->db->select('point_system,minpaid_get_point,point_get_per_minpaid,site_name,version,language_id,timezone,time_format,date_format')->where('id',1)->get('db_sitesettings');
            ...
            $this->data = array('theme_link'    => base_url().'theme/',
                                ...
                                'POINT_PER_PAID'              => $query->row()->point_get_per_minpaid,
                                'POINT_SYSTEM'              => $query->row()->point_system,
                                'POINT_PAID_VALUE'          =>  $query->row()->minpaid_get_point,
                                    );
      }
      public function load_global(){
            //Check login or redirect to logout
            if ($this->uri->segment(2) != 'public_invoice_aqua_home') {
